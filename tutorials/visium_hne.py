...
# This tutorial shows how to apply Squidpy for the analysis of Visium spatial transcriptomics dataset. The dataset used here consist of a Visium slide of a coronal section of the mouse brain. The original dataset is publicly availbale at the 10x genomics [dataset portal](https://support.10xgenomics.com/spatial-gene-expression/datasets). Here, we provide a pre-processed dataset, with pre-annoated clusters, in AnnData format. Couple of notes on pre-processing:
# - The pre-processing pipeline is the same as the one showed in the original [Scanpy tutorial](https://scanpy-tutorials.readthedocs.io/en/latest/spatial/basic-analysis.html)
# - The cluster annotation was performed using several resources, such as the [Allen Brain Atlas](http://mouse.brain-map.org/experiment/thumbnails/100048576?image_type=atlas), the Mouse Brain gene expression [atlas](http://mousebrain.org/genesearch.html) from the Linnarson lab and this recent [preprint](https://www.biorxiv.org/content/10.1101/2020.07.24.219758v1)
# 
...
# 
# Here, we provide a simple approach based on the well-knwon [Moran's I statistics](https://en.wikipedia.org/wiki/Moran%27s_I) which is in fact used also as a baseline method in the spatially variable gene papers listed above. The function in scanpy is called :func:`squidpy.gr.moran`, and returns both test statistics and adjusted pvalues in `adata.var` slot.
...
get_ipython().system('jupyter nbconvert --to python --no-prompt visium_hne.ipynb')
