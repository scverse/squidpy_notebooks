...
# This tutorial shows how to apply Squidpy to Imaging Mass Cytometry data. The data used here comes from a recent paper from `Jackson et al. (https://www.nature.com/articles/s41586-019-1876-x`_. We provide a pre-processed subset of the data, in AnnData format, that can be downloaded `here `_.
#
...
# ### Download data
# To get the data, you can simply run the following command, which downloads the relevant data from this `fighare link `_ and stores it in `~/.cache`.
...
# First, let's visualize cell type annotation in spatial context. For this, we can use the convenient function `scanpy.pl.spatial https://scanpy.readthedocs.io/en/stable/api/scanpy.pl.spatial.html`_ .
...
# You can familiarize yourself with network centralities from the excellent networkx `documentation https://networkx.org/documentation/stable/reference/algorithms/centrality.html?highlight=degree%20centrality#module-networkx.algorithms.centrality`_ . For the purpose of this analysis, we can appreciate that the *apoptotic tumor cell* clusters shows high closeness centrality, indicating that nodes belonging to that group are often close to each other in the spatial graph.
