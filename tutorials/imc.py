...
# You can run the notebooks in your own conda environemnt create
# your own with `conda create -f environment.yml`.
...
# For this, we can use the convenient function
...
# A first glimpse to the data already provides us with a lot of insights.
# For instance, we can appreciate how the majority of the tissue seems
...
# appreciate how a subset of tumor cell, *basal CK tumor cell* seems
# to regionalize in clusters in the lower part of the tissue.
#
# A key goal of Squidpy is to help analyst translates this intuitions
# on spatial patterns in quantitative scores that provides insight on
# tissue organization and nrighborhood structure.

# ### Co-occurrence across spatial dimensions
# In addition to the neighbor enrichemnt score, we can visualize cluster
# co-occurrence in spatial dimensions. This is a simialr analysis of the
...
# the argument `group`. Then, we visualize the results with
...
# the tissue is the *neighborhood nerichment test*.
...
# *endothelial cell*s, as well as *macrophages*. Another interesting
...
# more likley to be enriched with cell types from the same class,
# rather than different one.
#
# This is in constrast with a parallel analysis that can be
# performed with Squidpy, with the function :func:`squidpy.gr.interaction_matrix`.
# This method builds an interaction matrix based on the number of edges
# that each cluster share with all the others. By visualizing the results
# with  :func:`squidpy.pl.interaction_matrix`, we can appreciate that
# *apoptotic tumor cells* share indeed many edges with other annotations,
# being uniformly spread across the tissue area.
# This is a descriptive statistics, but nonetheless useful
# to understand organization of cell types in tissue.
...
# ### Centrality scores
# Finally, similar to the previous analysis, we can
# investigate properties of the spatial graph by
# computing different network centralities network centralities:
# - degree_centrality
# - average_clustering
